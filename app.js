...
async function loadData(){
  try{
    const [ir,pr,mr]=await Promise.all([
      fetch(`data/teamkills.json?v=${Date.now()}`),
      fetch(`data/players.json?v=${Date.now()}`),
      fetch(`data/map-images.json?v=${Date.now()}`)
    ]);
    if(!ir.ok)throw Error(`HTTP ${ir.status}`);
    const incidentData=await ir.json();
    state.incidents=Array.isArray(incidentData)?incidentData:[];
    state.players=pr.ok?normalizePlayers(await pr.json()):[];
    state.mapImages=mr.ok?await mr.json():[];
    if(!Array.isArray(state.mapImages))state.mapImages=[];
    updateTime();
  }catch(e){
    console.error(e);
    state.incidents=[];
...
function mapNameHtml(name){
  ...
  return`<span class="player-hover map-hover" tabindex="0">${esc(shown)}<span class="player-hover-card"><img src="${esc(`assets/maps/${file}`)}" alt="${esc(shown)} map" loading="lazy" onerror="this.closest('.player-hover').classList.add('image-error')"><span class="player-hover-name">${esc(shown)}</span></span></span>`;
}
