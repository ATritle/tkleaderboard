...
function renderLeaderboard(){
  ...
  $('leaderboard').innerHTML=names.length?`<div class="rank-card table-label"><div>#</div><div>PLAYER</div><div class="rank-count">TEAM KILLS</div><div class="rank-count">VICTIMS</div><div class="rank-count">VICTIMIZED</div><div class="rank-count">TK RATIO</div></div>`+names.map((name,idx)=>{const kills=kc[name]||0,victims=new Set(all.filter(i=>norm(i.killer)===name).map(i=>norm(i.victim))).size,received=vc[name]||0,total=kills+received,ratio=total?Math.round(kills/total*100):0;return `<div class="rank-card"><div class="rank">${idx+1}</div><div class="rank-name">${esc(titleCase(name))}</div><div class="rank-count"><strong>${kills}</strong></div><div class="rank-count"><strong>${victims}</strong></div><div class="rank-count"><strong>${received}</strong></div><div class="rank-count"><strong>${ratio}%</strong></div></div>`}).join(''):'<div class="empty"><p>No incidents recorded yet.</p></div>';
}
...
function incidentRow(i){const clip=i.clip?`<a class="clip-icon" href="${esc(i.clip)}" target="_blank" rel="noopener" title="View clip">↗</a>`:'';return `<div class="incident-row"><span class="tkid">${esc(i.id||'')}</span><span class="killer">${esc(titleCase(i.killer))}</span><span class="victim">${esc(titleCase(i.victim))}</span><span class="date">${formatDate(i.date)}</span><span class="map">${esc(i.map||'Unknown')}</span><span class="notes" title="${esc(i.notes||'')}">${esc(i.notes||'No notes recorded.')}</span><span>${clip}</span></div>`}
